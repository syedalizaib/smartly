...
class QuizPlayViewModel(private val repository: Repository) : ViewModel() {
    ...
    fun fetchPlayQuestions() {
        loadingBarIsVisible.value = true
        viewModelScope.launch {
            supervisorScope {
                val questionCall = async {
                    val questionRemoteResponse = repository.getRemoteQuestions(
                        Constants.QUESTION_COUNT.toString(),
                        categoryID.value!!,
    ...
    fun nextQuestion() {
        try {
            if (!questionList.value!!.isEmpty()) {
                ...
                quizQuestionsAttempted.value = 5 - questionList.value!!.size
                setButtonViews()
                isButtonEnabled.value = true
                tickerMiliseconds.value = 6000
    ...
    fun setButtonViews() {
        isMcq.value = !questionType.value.equals(Constants.QUESTION_TYPE_BOOLEAN)
    }
    ...
    fun incrementScore() {
        ...
        updateCatScore(categoryID.value!!.toInt(), incVal)
    }
